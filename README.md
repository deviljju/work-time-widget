...
- 항상 위에 표시 옵션
- 상단 영역 드래그로 위젯 이동
- 작업표시줄 없이 시스템 트레이에 상주
- 단일 인스턴스 실행 및 Windows 로그인 시 자동 시작
...
닫기 버튼을 누르면 앱이 종료되지 않고 시스템 트레이로 숨겨집니다. 트레이 아이콘을 왼쪽 클릭하면 위젯이 다시 열리며, 오른쪽 클릭 메뉴에서 `위젯 열기`, `설정`, `종료`를 사용할 수 있습니다.

Windows 로그인 시 자동 실행은 설정의 `Windows 시작 시 실행`에서 관리합니다. 활성화하면 현재 사용자 Startup 폴더에 바로가기가 생성되고, 비활성화하면 제거됩니다.
